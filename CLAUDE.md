...
- **Week 3** — [week03-tool-calling/](week03-tool-calling/) — `app.py` implements the full tool-calling round trip: call model with `tools=[...]` (schemas built via `openai.pydantic_function_tool()` from `WeatherInput`/`WikipediaInput` in `models.py`) → parse `response.choices[0].message.tool_calls` → re-validate the model's JSON args with the matching Pydantic model before dispatch → run the real Python function (`tools.py`: `get_weather` hits wttr.in, `search_wikipedia` hits Wikipedia's search API) → append the full assistant turn + a `role: tool` result message back into `messages` → second API call produces the final natural-language answer. Confirmed working end-to-end for both tools (weather + Wikipedia queries manually tested). Bugs hit + fixed: `from sympy import re` silently shadowed stdlib `re`, breaking `re.sub()` in `search_wikipedia`; `role: tool` message content sent as a raw dict instead of `json.dumps()`'d string (API requires string/array content); assistant turn re-appended with only `content` (dropping `tool_calls`), breaking the model's memory of its own tool request on the next call; a leftover system-prompt line ("always output JSON with the provided schema") from an earlier design using `response_format` caused the model to hallucinate a fake `"JSON"` tool call once `response_format` was removed but the instruction wasn't; Wikipedia blocked the default `requests` User-Agent (403, plain-text body) and `search_wikipedia` had no status-code check before `.json()`, unlike `get_weather` — all logged in "Recurring small-detail gaps" below.
